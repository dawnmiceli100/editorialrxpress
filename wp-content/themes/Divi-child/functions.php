...
/* DMM added code to enable divi builder on all post types */
...
function myprefix_add_meta_boxes() {
  foreach(get_post_types() as $pt) {
    if (post_type_supports($pt, 'editor') and function_exists('et_single_settings_meta_box')) {
      add_meta_box('et_settings_meta_box', esc_html__('Divi News Settings', 'Divi'), 'et_single_settings_meta_box', $pt, 'side', 'high');
    }
  } 
}
...
/* END DMM added code to enable divi builder on all post types */
?>